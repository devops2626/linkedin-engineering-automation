...
def validate_document(document_path: Path) -> None:
    """Pre-flight checks before attempting upload. Raises DocumentValidationError on failure."""
...
def _request_with_retry(
    ...
    **kwargs,
    ...
    """HTTP request with limited retries on transient failures (5xx, connection, timeout)."""
        ...
        try:
            ...
            if resp.status_code >= 500:
                logger.warning(
                    ...
                    attempt, max_retries, resp.status_code, method, url,
                )
                last_exc = HTTPError(
...
def initialize_document_upload(owner: str) -> dict:
    ...
    try:
        resp = _request_with_retry(
            "POST", url, json=payload, headers=_api_headers(), timeout=30,
        )
...
def get_document_status(document_urn: str) -> str:
    ...
    try:
        resp = _request_with_retry(
            "GET", url, headers=_api_headers(content_type=None), timeout=30,
        )
    ...
    while True:
        ...
        if status == "STATUS_FETCH_FAILED":
            ...
            logger.warning(
                ...
                consecutive_fetch_failures, max_consecutive_fetch_failures,
            )
...
def upload_and_prepare_document(document_path: Path, owner: str) -> str:
    """Full document lifecycle: validate → initialize → upload → poll."""
...
def post_to_linkedin(
    ...
    *,
    ...
    """Publish a text-only or native document post."""
    require_credentials()
    ...
    if document_urn:
        payload["content"] = {
            "media": {"id": document_urn, "title": document_title or "Document"}
        }
        logger.info(
            "Creating native document post (urn=%s, title=%s)",
            document_urn, document_title,
        )
    else:
